...
class ParaBench {
  ...
  probe (options, userCode) {
    ...
    }).then( arg => timedPromise( 'Probe', options.timeout, resolve => {
  ...
  check(timeout = 1, arg = 1) {
     ...
     const iterate = (resolve) => {
       ...
       this.probe({arg, timeout}, this._solution[name])
    ...
    const iterate = resolve => {
      ...
      return this.probe({arg: n}, variants[name]).then(piece => {
        ...
        this._progress({
          ...
          result: piece,
          totalTime: cpu,
          percent: Math.min(100, 100 * cpu / maxTime ),
        });
        iterate(resolve);
      });
    }
